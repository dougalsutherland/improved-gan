...
def convert(x):
    ...
    if args.scale_full:  # data is in [0, 1], scale to [-1, 1]
        t *= 2
        t -= 1
    return t

with np.load(os.path.join(os.path.dirname(__file__), 'mnist.npz')) as d:
    trainx = convert(np.r_[d['x_train'], d['x_valid']])
    testx = convert(d['x_test'])
    trainy = np.r_[d['y_train'], d['y_valid']].astype(np.int32)
    testy = d['y_test'].astype(np.int32)
